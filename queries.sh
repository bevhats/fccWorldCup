...
echo "$($PSQL "SELECT MAX(winner_goals) AS max_win_goals FROM games")"
...
echo "$($PSQL "SELECT teams.name FROM teams LEFT JOIN games ON  teams.team_id = games.winner_id OR teams.team_id = games.opponent_id WHERE year = 2014 AND round = 'Eighth-Final' ORDER BY teams.name ASC")"
...
echo "$($PSQL "SELECT g.year, t.name FROM games as g LEFT JOIN teams as t ON g.winner_id = t.team_id  WHERE round = 'Final' ORDER BY year ASC")"
